...
class MainRecommender:
    ...
    def __init__(self, data: pd.DataFrame, weighting: bool = True):
        ...
        self.user_item_matrix_tfidf = self._prepare_matrix(data)
        self.user_item_matrix_tfidf = tfidf_weight(self.user_item_matrix_tfidf.T).T
        
        self.user_item_matrix_bm25 = self._prepare_matrix(data)
        ...
        if weighting:
            self.user_item_matrix = bm25_weight(self.user_item_matrix.T, K1=60, B=0.6).T
#             self.user_item_matrix = bm25_weight(self.user_item_matrix.T).T
    ...
    def get_als_tfidf_recommendations(self, user, N=5):
        """Рекомендуем топ-N товаров"""
        self._update_dict(user_id=user)
        res = [self.id_to_itemid[rec[0]] for rec in 
                        self.model_tfidf.recommend(userid=self.userid_to_id[user], 
                                                 user_items=csr_matrix(self.user_item_matrix_tfidf).tocsr(), 
                                                 N=N, 
                                                 filter_already_liked_items=False, 
                                                 filter_items=[self.itemid_to_id[999999]],  # !!! 
    ...
    def get_als_bm25_recommendations(self, user, N=5):
        ...
        res = [self.id_to_itemid[rec[0]] for rec in 
                        self.model_bm25.recommend(userid=self.userid_to_id[user], 
                                                 user_items=csr_matrix(self.user_item_matrix_bm25).tocsr(), 
                                                 N=N, 
                                                 filter_already_liked_items=False, 
                                                 filter_items=[self.itemid_to_id[999999]],  # !!! 
                                                 recalculate_user=True)]
